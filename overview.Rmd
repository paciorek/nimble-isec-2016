---
title: "Flexible programming with BUGS models: Using NIMBLE for MCMC and beyond"
subtitle: "ISEC 2016 workshop"
...
The Github site ([https://github.com/paciorek/nimble-isec-2016](https://github.com/paciorek/nimble-isec-2016)) is the master repository for materials for the modules; you can also get the individual files from Github. 

To get the materials look ahead to slide 6 for instructions. Alternatively, simply go to [https://github.com/paciorek/nimble-isec-2016](https://github.com/paciorek/nimble-isec-2016) and click on the 'Download ZIP' button in the right side of the window or simply click [here](https://github.com/paciorek/nimble-isec-2016/archive/master.zip).



## Asking questions and making comments

Please ask questions and make comments as we go through the material via the online shared document (Etherpad) at [https://public.etherpad-mozilla.org/p/nimble-isec-2016](https://public.etherpad-mozilla.org/p/nimble-isec-2016). You can also share ideas, code snippets, and error messages.

# How we'll operate

The workshop will be organized in modules, each of which will be a combination of lecture/demo presentation, in some cases followed by a short breakout session in which you'll have a chance to work on some of the problems in the *exercises.html* file.
...
If you want to access the R code shown in each HTML module separately, you can find that in *foo.R*. 
...
Simply go to [https://github.com/paciorek/nimble-isec-2016](https://github.com/paciorek/nimble-isec-2016) and click on the 'Download ZIP' button in the right side of the window or simply click [here](https://github.com/paciorek/nimble-isec-2016/archive/master.zip).

## Within RStudio

Within RStudio go to File->New Project->Version Control->Git and enter:

- "Repository URL": https://github.com/paciorek/nimble-isec-2016
- "Project Directory Name": nimble-isec-2016 (or something else of your choosing)
...
- `git clone https://github.com/paciorek/nimble-isec-2016`

Then to update from the repository to get any changes we've made:

- `cd /directory/where/you/put/the/repository/nimble-isec-2016`
- `git pull`

# Installing NIMBLE

NIMBLE is not yet on CRAN (but should be very soon!), so you need to download it from our website, [http://r-nimble.org/download](http://r-nimble.org/download). More details can also be found in Section 4 of the [NIMBLE manual](http://r-nimble.org/manuals/NimbleUserManual.pdf). 
...
* NIMBLE can take a long time to build models that have tens of thousands or more nodes (though once built, the algorithm run times can be quite good). The next release of NIMBLE (v. 0.6) should provide some big improvements in this regard.
...
Here are the modules we'll work through for this workshop, though we probably won't get to all of them.
 ...
 - How NIMBLE is different than other packages
    ...
    6. [How you could use NIMBLE](how_you_could_use_nimble_slides.html)
    7. [Exercises (Using and customizing MCMC)](exercises.html)
 - NIMBLE programming: models and algorithms
    8. [Programming with models (I): Operating a NIMBLE model](operating_model_slides.html)
    9. [Programming with models (II): Understanding and querying model structure](model_structure_slides.html)
    10. [Rewriting the model for more efficient MCMC](thinking_like_a_graph_slides.html)    
    11. [nimbleFunction programming (I): Compiling simple math from R to C++](compile_R_slides.html)
    12. [Exercises (Operating a model and compiling R code)](exercises.html)
 - NIMBLE programming: models and algorithms in action
    13. [Providing new distributions for use in BUGS](user_dist_slides.html)
    14. [Capture-recapture example: a user-defined distribution for multi-state capture histories](user_dist_DHMM_slides.html)
    15. [nimbleFunction programming (II): model-generic two-stage programming](nimbleFunctions_slides.html)
    16. [Spatial capture-recapture example: Writing new MCMC samplers](user_sampler_spatial_capture_recapture_slides.html)
    17. [Exercises (using nimbleFunctions)](exercises.html)
 - Writing and using other algorithms:
    18. [Maximum likelihood with MCEM](run_mcem_slides.html)
    19. [Sequential Monte Carlo (aka particle filtering)](particle_filter_slides.html)
...
2. [Data nodes](data_nodes_slides.html)
3. [Indexing](indexing_slides.html)
4. [User-defined BUGS functions](user_function_slides.html)
5. [An extended MCMC example](litters_example_slides.html)
